---
title: "Schema Changes in ObjectBox Golang"
description: "ObjectBox manages its data model (schema) mostly automatically. ObjectBox db supports automatic schema migration to make data persistence as easy as possible..."
keywords:
  - objectbox
  - database
  - schema
...
<TechnicalArticleSchema
  headline="Schema Changes in ObjectBox Golang"
  description="ObjectBox manages its data model (schema) mostly automatically. ObjectBox db supports automatic schema migration to make data persistence as easy as possible..."
  url="https://golang.objectbox.io//schema-changes"
...
:::info
Note how for a property, the output is slightly different and, besides support for renaming, it provides a newly generated UID you can use to effectively reset (clean) the stored data on the property. See [Reset data - new UID on a property](schema-changes#reset-data-new-uid-on-a-property) for more details.
:::
